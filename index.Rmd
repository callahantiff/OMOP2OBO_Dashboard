...
# download data sets
# condition data
core_url = 'https://zenodo.org/record/6929364/files/OMOP2OBO_'
...
# drug data
core_url = 'https://zenodo.org/record/6929377/files/OMOP2OBO_'
...
# measurement data
core_url = 'https://zenodo.org/record/6929384/files/OMOP2OBO_'
...
cond_count <- n_distinct(cond_data$CONCEPT_ID)
drug_count<- n_distinct(drug_data$CONCEPT_ID)
lab_count <- n_distinct(lab_data$CONCEPT_ID)
...
aca = "Automatic Constructor - Ancestor"
acc = "Automatic Constructor - Concept"
aea = "Automatic Exact - Ancestor"
aec = "Automatic Exact - Concept"
m = "Manual"
mc = "Manual Constructor"
...
names(c_out_plot)[names(c_out_plot) == "Manual Exact - Concept Similarity"] <- "Concept Similarity"
names(c_out_plot)<-str_replace_all(names(c_out_plot), c(" " = "." , "-" = "." ))
c_out_plot[is.na(c_out_plot)] <- 0

# create plot data
onts <- c_out_plot$ONTOLOGY
aca_df <- c_out_plot$Automatic.Constructor...Ancestor
acc_df <- c_out_plot$Automatic.Constructor...Concept
aea_df <- c_out_plot$Automatic.Exact...Ancestor
aec_df <- c_out_plot$Automatic.Exact...Concept
m_df <- c_out_plot$Manual
mc_df <- c_out_plot$Manual.Constructor
cs_df <- c_out_plot$Concept.Similarity
...
datatable(c_out_table, filter = "top")
...
# rename casted df and convert n/a values to 0
names(d_out_plot)[names(d_out_plot) == "Manual Exact - Concept Similarity"] <- "Concept Similarity"
names(d_out_plot)<-str_replace_all(names(d_out_plot), c(" " = "." , "-" = "." ))
d_out_plot[is.na(d_out_plot)] <- 0

# create plot data
onts <- d_out_plot$ONTOLOGY
aca_df <- d_out_plot$Automatic.Constructor...Ancestor
acc_df <- d_out_plot$Automatic.Constructor...Concept
aea_df <- d_out_plot$Automatic.Exact...Ancestor
aec_df <- d_out_plot$Automatic.Exact...Concept
m_df <- d_out_plot$Manual
mc_df <- d_out_plot$Manual.Constructor
cs_df <- d_out_plot$Concept.Similarity
...
datatable(d_out_table, filter = "top")
...
# rename casted df and convert n/a values to 0
names(l_out_plot)[names(l_out_plot) == "Manual Exact - Concept Similarity"] <- "Concept Similarity"
names(l_out_plot)<-str_replace_all(names(l_out_plot), c(" " = "." , "-" = "." ))
l_out_plot[is.na(l_out_plot)] <- 0

# create plot data
onts <- l_out_plot$ONTOLOGY
aca_df <- l_out_plot$Automatic.Constructor...Ancestor
acc_df <- l_out_plot$Automatic.Constructor...Concept
aea_df <- l_out_plot$Automatic.Exact...Ancestor
aec_df <- l_out_plot$Automatic.Exact...Concept
m_df <- l_out_plot$Manual
mc_df <- l_out_plot$Manual.Constructor
cs_df <- l_out_plot$Concept.Similarity
...
datatable(l_out_table, filter = "top")
...
cond_data_subset <- cond_data[1:10000, ]

# make table
datatable(cond_data_subset, filter = "top", options = list(
  pageLength = 10,
...
drug_data_subset <- drug_data[1:10000, ]

# make table
datatable(drug_data_subset, filter = "top", options = list(
  pageLength = 10,
  lengthMenu = c(10, 25, 50, 100, 500),
  searchHighlight = TRUE))
```
...
# make table
datatable(lab_data, filter = "top", options = list(
  pageLength = 10,
  lengthMenu = c(10, 25, 50, 100, 500),
  searchHighlight = TRUE))
```
